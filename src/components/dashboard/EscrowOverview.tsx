...
export const EscrowOverview: React.FC = () => {
  ...
  const fetchEscrows = async () => {
    try {
      ...
      const total = escrowList.reduce((sum: number, e: Escrow) => {
        const amt = typeof e.amount === 'string' ? parseFloat(e.amount as string) : (e.amount || 0);
        const n = Number.isFinite(Number(amt)) ? Number(amt) : 0;
        return sum + n;
      }, 0);
      const funded = escrowList
        .filter((e: Escrow) => e.status === 'funded')
        .reduce((sum: number, e: Escrow) => {
          const amt = typeof e.amount === 'string' ? parseFloat(e.amount as string) : (e.amount || 0);
          const n = Number.isFinite(Number(amt)) ? Number(amt) : 0;
          return sum + n;
        }, 0);
        ...
        {escrows.slice(0, 3).map((escrow) => {
          ...
          return (
            ...
            >
              ...
              <div className="text-right">
                <p className="font-semibold text-[#141414]">{formatCurrency(typeof escrow.amount === 'string' ? parseFloat(String(escrow.amount)) : Number(escrow.amount || 0))}</p>
