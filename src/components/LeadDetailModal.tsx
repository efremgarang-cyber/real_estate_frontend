...
import { X, CreditCard, MessageSquare, History, Loader2, Plus } from "lucide-react";
...
export const LeadDetailModal: React.FC<LeadDetailModalProps> = ({ leadId, onClose }) => {
  ...
  if (loading) {
    return (
      <div className="fixed inset-0 z-[100] bg-[#141414]/40 backdrop-blur-sm flex items-center justify-center">
  ...
  return (
    <motion.div 
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className="fixed inset-0 z-[100] bg-[#141414]/40 backdrop-blur-sm flex items-center justify-end p-4 md:p-6 font-sans"
      onClick={onClose}
    >
      <motion.div 
        ...
        className="w-full max-w-2xl h-full bg-white rounded-[2rem] shadow-2xl overflow-y-auto flex flex-col"
      ...
      >
        <div className="p-8 border-b border-gray-100 flex items-center justify-between sticky top-0 bg-white/80 backdrop-blur-md z-10">
          ...
          <button onClick={onClose} className="p-2 text-gray-400 hover:bg-gray-100 hover:text-[#141414] rounded-full transition-colors">
            <X size={24} />
          </button>
        </div>
        ...
        <div className="p-8 space-y-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="space-y-8">
              <div className="bg-white border border-gray-100 shadow-[0_10px_30px_rgba(0,0,0,0.02)] rounded-[2rem] p-6">
