...
import { X, ArrowRight, Upload, Trash2, ShieldAlert } from "lucide-react";
...
interface NewListingModalProps {
  onClose: () => void;
  onSuccess: () => void;
  userRole?: string; // <-- Fixed: Changed from required to optional with '?' so it stops breaking Listings.tsx
}

export const NewListingModal: React.FC<NewListingModalProps> = ({ onClose, onSuccess, userRole }) => {
  ...
  // RBAC Validation: If userRole is omitted or unauthenticated, default to restricting access
  const isAuthorized = userRole === "agent" || userRole === "admin";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Explicit guard clause check before triggering API stream calls
    if (!isAuthorized) {
      alert("Unauthorized operational action execution.");
      return;
    }
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Conditional Rendering: Show warning if regular client, show form if Agent/Admin */}
        {!isAuthorized ? (
          <div className="flex flex-col items-center justify-center text-center py-6 space-y-4">
            <div className="p-4 bg-amber-50 text-amber-700 rounded-full">
              <ShieldAlert size={40} />
            </div>
            <div className="space-y-2">
              <h4 className="font-bold text-lg text-[#141414]">Access Restricted</h4>
              <p className="text-sm text-gray-500 max-w-md">
                Public user profiles do not have listing permissions. Please initialize or join an Agency workspace to register as an active agent and publish properties.
              </p>
            </div>
            <button
              type="button"
              onClick={onClose}
              className="px-6 py-2.5 bg-[#141414] hover:bg-black text-white text-sm font-medium rounded-xl transition-colors"
            >
              Return to Listings
            </button>
          </div>
