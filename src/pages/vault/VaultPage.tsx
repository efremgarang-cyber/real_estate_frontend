...
export const VaultPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchDocuments = async () => {
      setLoading(true);
      try {
        const response = await documentApi.getAgencyDocuments({
          agencyId: String(profile?.agencyId || ""),
        });
  ...
  const handleDocumentSave = async () => {
    setLoading(true);
    try {
      const response = await documentApi.getAgencyDocuments({ agencyId: String(profile?.agencyId || "") });
      setDocs(response.data?.data || []);
      setShowUploadModal(false);
    } catch (error) {
      console.error("Failed to refresh documents after upload", error);
      alert("Uploaded but failed to refresh document list.");
    } finally {
      setLoading(false);
    }
  };
