...
export const LandingPage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-white font-sans text-[#141414]">
      ...
      <section className="bg-gradient-to-br from-[#141414] to-[#1a1a1a] text-white relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2">
          <div className="p-12 md:p-16 flex flex-col justify-center relative z-10">
            ...
            <button 
              onClick={() => navigate("/auth/login")}
              className="cursor-pointer mt-12 w-fit bg-[#D4AF37] text-[#141414] px-10 py-4 font-bold uppercase tracking-wider text-sm hover:bg-white transition-all duration-300 hover:shadow-2xl transform hover:-translate-y-0.5 rounded"
            >
              Request Assessment
            </button>
          </div>
      ...
      {/* ─── 6. FAQ SECTION — Accordion like Safaricom ──────────────────────────────────────────── */}
      <section id="faq-section" className="py-24 px-6 md:px-12 bg-gray-50">
        <div className="max-w-3xl mx-auto">
          ...
          <div className="space-y-3">
            {[
              {
                q: "How do I start using Makao?",
                a: "Simply click 'Get Started' to create your account. Once registered, you can browse verified properties, list your own, and connect with professional agents — all within a secure platform."
              },
              {
                q: "Is my personal and financial data secure?",
                a: "Absolutely. Makao uses bank-grade encryption and secure escrow services for all transactions. Your documents are stored in encrypted cloud storage with restricted access controls."
              },
              {
                q: "How does the escrow payment system work?",
                a: "When you agree on a deal, funds are held securely in escrow until all conditions are met. Milestones release payments only after verification — protecting both buyers and sellers throughout the transaction."
              },
              {
                q: "What documents do I need for KYC verification?",
                a: "You'll need a valid government-issued ID (national ID or passport), a KRA PIN certificate, and a proof of address (utility bill or bank statement). Upload them securely through your dashboard."
              },
              {
                q: "Can I list multiple properties?",
                a: "Yes. Agents and property owners can list multiple properties. Each listing goes through a verification process to ensure accuracy and authenticity before being published."
              },
              {
                q: "How do I contact support?",
                a: "You can reach our support team via phone at +254 (0) 700 000 000 or email at acquisitions@makao.co.ke. Our team is available Monday to Friday, 8:00 AM to 6:00 PM EAT."
              }
            ].map((faq, index) => {
              const isOpen = openFaqIndex === index;
              return (
                <div key={index} className="bg-white rounded-2xl border border-gray-200 overflow-hidden transition-all duration-300 hover:border-[#D4AF37]/30">
                  <button
                    onClick={() => setOpenFaqIndex(isOpen ? null : index)}
                    className="w-full flex items-center justify-between p-5 md:p-6 text-left"
                  >
                    <span className="text-sm md:text-base font-bold text-[#141414] pr-4">{faq.q}</span>
                    <ChevronRight
                      size={18}
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-12 text-xs">
            <div className="flex flex-col gap-3 text-gray-400">
              ...
              {["Residential Sales", "Commercial Leasing", "New Developments", "Land Plots"].map((l) => (
                <span key={l} onClick={() => navigate("/properties")} className="cursor-pointer hover:text-[#D4AF37] transition-colors duration-300">
                  {l}
                </span>
              ))}
            </div>
            <div className="flex flex-col gap-3 text-gray-400">
              ...
              <span onClick={() => navigate("/properties")} className="cursor-pointer hover:text-[#D4AF37] transition-colors duration-300">
                Buyer's Guide
              </span>
              <span onClick={() => navigate("/properties")} className="cursor-pointer hover:text-[#D4AF37] transition-colors duration-300">
                Seller's Guide
            ...
            <div className="flex flex-col gap-3 text-gray-400">
              ...
              <span onClick={() => navigate("/auth/login")} className="cursor-pointer hover:text-[#D4AF37] transition-colors duration-300">
                KYC Verification
              </span>
              <span onClick={() => window.open("https://laws.africa/", "_blank")} className="cursor-pointer hover:text-[#D4AF37] transition-colors duration-300">
                Legal Framework
              </span>
            </div>
          </div>
