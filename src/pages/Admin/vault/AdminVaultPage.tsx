...
export const AdminVaultPage: React.FC = () => {
  ...
  const filteredDocs = docs.filter(doc => {
    const matchesSearch = 
      ...
      String(doc.agentName || '').toLowerCase().includes(searchTerm.toLowerCase());
                    ...
                    >
                      ...
                      <td className="px-6 py-4.5">
                        <div className="flex items-center gap-3">
                            <div className="w-8 h-8 rounded-xl bg-[#141414] text-white flex items-center justify-center font-black text-[10px] tracking-wider shadow-sm">
                              {String(targetClient).slice(0, 2).toUpperCase()}
                            </div>
                            <div>
                              <span className="font-bold text-gray-900 text-xs block">{targetClient}</span>
                              <span className="text-[9px] font-mono text-gray-400 block mt-0.5">DOC-ID: {String(doc.id).substring(0, 8).toUpperCase()}</span>
                            </div>
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-4.5">
                        <div className="flex items-center gap-2 text-xs font-semibold text-gray-700">
                          ...
                          <span className="capitalize">{String(doc.type || '').replace(/_/g, ' ')}</span>
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-4.5 text-xs font-medium text-gray-400 font-mono">
                        {new Date(
                          (doc as any).updated_at || (doc as any).updatedAt || (doc as any).createdAt || (doc as any).created_at || Date.now()
                          ...
                          <div className="flex items-center justify-end gap-1 opacity-80 group-hover:opacity-100 transition-opacity">
                            ...
                            >
                              ...
                              {String(doc.status) !== "approved" && (
                                <button
                                  onClick={() => handleUpdateDocumentStatus(String(doc.id), "approved")}
                              ...
                              {String(doc.status) !== "rejected" && (
                                <button
                                  onClick={() => handleUpdateDocumentStatus(String(doc.id), "rejected")}
        ...
        {selectedDoc && (
            <DocumentViewer
              ...
              onUpdateStatus={async (status: any) => {
                await handleUpdateDocumentStatus(String(selectedDoc.id), status);
                setSelectedDoc(null);
              }}
            />
        )}
