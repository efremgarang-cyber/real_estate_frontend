...
export const AdminPropertyDetails: React.FC = () => {
  ...
  // Upload images mutation - FIXED: Uses attachImage endpoint instead of constructing fake URLs
  const uploadImagesMutation = useMutation({
    mutationFn: async (files: FileList) => {
      ...
      for (const file of Array.from(files)) {
        // Upload to Supabase and get the internal storage path
        const s3Path = await vaultApi.executeSecureUpload(file, 'property_image');
        
        // Attach the image to the property via the backend API
        // This creates a PropertyImage record with the s3_path
        const result = await propertyApi.attachImage(property.id, s3Path);
    ...
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ["adminPropertyDetails", id] });
      addNotification('success', 'Images Uploaded', `${data.length} image(s) added successfully.`);
...
const deletePropertyMutation = useMutation({
  ...
  // Image upload handler - uploads to Supabase then attaches via backend API
  ...
  // Handle image upload button click (always available, not just in edit mode)
    ...
    <div className="space-y-6 font-sans text-[#141414] pb-12">
      ...
      <div className="space-y-4">
        ...
        <div className="relative aspect-[21/9] rounded-2xl overflow-hidden border border-neutral-200 bg-gray-100 group">
          ...
          {/* Image Upload Overlay - Always visible, not just in edit mode */}
          ...
          <div className="flex gap-2 overflow-x-auto pb-2">
            {images.map((img, idx) => (
              <div key={idx} className="relative group/image flex-shrink-0">
                ...
                {/* Show delete button for non-primary images */}
                {idx !== 0 && imageUrls && imageUrls[idx] && (
                  <button
  ...
  className="px-6 py-3 bg-red-600 hover:bg-red-700 text-white rounded-xl text-sm font-bold transition-all shadow-sm disabled:opacity-50"
  disabled={forceDeleteMutation.isPending || deletePropertyMutation.isPending}
>
  {forceDeleteMutation.isPending ? (
    ...
    <span className="flex items-center gap-2"><Trash2 size={16} /> Permanently Delete Property</span>
  )}
</button>
        </div>
      </div>
