...
import { Building2, ArrowRight, Eye, EyeOff, AlertCircle, ShieldCheck } from "lucide-react";
...
export const AdminLoginPage: React.FC = () => {
  ...
  const handleAuthSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isSignUp) {
        // Enforce the 'admin' account type context on registration 
        await register(
          email, 
          password, 
          name, 
  ...
  const handleCreateAgency = async () => {
    ...
    setAgencyError(null);
    try {
      // Force "Admin" tier role context inside the tenant generation engine
      ...
      <div className="min-h-screen flex items-center justify-center bg-[#E4E3E0] p-6 font-sans">
        <div className="max-w-md w-full p-10 bg-white rounded-[2rem] shadow-[0_20px_50px_rgba(0,0,0,0.05)]">
          ...
          <h1 className="font-display text-2xl font-bold text-center text-[#141414] mb-2">
            {isSignUp ? "Admin Registration" : "Admin Console Login"}
          </h1>
          ...
          <form onSubmit={handleAuthSubmit} className="space-y-5 text-left">
            {isSignUp && (
              <>
                <div className="p-3 bg-neutral-50 border border-neutral-200/60 rounded-xl flex items-start gap-2.5 animate-in fade-in slide-in-from-top-2 duration-200">
                  <ShieldCheck size={16} className="text-neutral-700 shrink-0 mt-0.5" />
                  <p className="text-xs text-neutral-500 leading-normal">
                    Creating an <strong>Agency Admin</strong> profile creates a tenant sandbox. Next, you will establish your workspace & configure billing access parameters.
                  </p>
                </div>
            ...
            >
              {isAuthenticating ? "Processing..." : isSignUp ? "Register Master Workspace" : "Console Secure Sign In"}
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#E4E3E0] p-6 font-sans">
        <div className="max-w-md w-full p-10 bg-white rounded-[2rem] shadow-[0_20px_50px_rgba(0,0,0,0.05)]">
          ...
          <p className="text-sm text-center text-gray-500 mb-6">
            Welcome, <span className="font-semibold text-[#141414]">{user.name || 'Admin'}</span>.
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#E4E3E0]">
      <p className="text-sm font-medium text-gray-500 animate-pulse">Routing security clearance...</p>
    </div>
  );
};