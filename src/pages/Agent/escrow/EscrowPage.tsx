...
export const EscrowPage: React.FC = () => {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      // 1) Create the escrow record
      const createResp = await escrowApi.create({
        property_id: undefined,
        amount: parseFloat(amount),
        terms: form.propertyTitle || 'Property escrow',
      });

      console.debug('escrow.create response:', createResp);
      const createdEscrow = createResp.escrow || createResp.data || createResp || null;
      const createdId = createdEscrow?.id || createdEscrow?.escrow_id || null;

      if (!createdId) {
        throw new Error('Escrow creation failed: no id returned');
      }

      // Refresh to surface the created escrow in the UI list
      await refreshTransactions?.();
      triggerToast('Escrow created. Initializing payment...', 'info');

      // 2) Initialize Paystack payment for the created escrow
      try {
        const initResp = await api.post('/paystack/initialize', {
          amount: parseFloat(amount),
          escrow_id: createdId,
        });
        const authUrl = initResp.data?.authorization_url || initResp.data?.authorizationUrl || initResp.data?.authorization_url;
        const altUrl = initResp.data?.authorization_url || initResp.data?.authorizationUrl || initResp.data?.authorization?.authorization_url || null;

        const redirectUrl = authUrl || altUrl || initResp.data?.authorization_url;

        if (redirectUrl) {
          triggerToast('Redirecting to payment gateway...', 'success');
          window.location.href = redirectUrl;
          return;
        } else {
          triggerToast('Payment initialized but no redirect URL returned. Refreshing list.', 'info');
          await refreshTransactions?.();
        }
      } catch (payErr: any) {
        console.error('Payment initialization failed:', payErr);
        triggerToast('Payment initialization failed. Escrow created.', 'error');
        await refreshTransactions?.();
      }
    } catch (err: any) {
  ...
  // ─── Handle deposit ──────────────────────────────────────────
const handleDepositInitialization = async (amount: number, escrowId?: number | string): Promise<string | null> => {
  try {
    ...
    const targetEmail = form.clientEmail || sessionUser?.email || 'agent@makao.com';
     ...
     if (!escrowId) {
       throw new Error('Cannot initialize deposit without an escrow reference.');
     }
  ...
  const handlePaymentClick = async (escrowData: EscrowWithProgress) => {
    ...
    try {
      const resp = await api.post('/paystack/initialize', {
        amount: remaining,
        escrow_id: escrow.id,
      });

      const redirect = resp.data?.authorization_url || resp.data?.authorizationUrl || resp.data?.authorization?.authorization_url || null;
      if (redirect) {
